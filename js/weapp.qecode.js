...
/**
 * weapp.qrcode.js v1.1.2 https://developers.weixin.qq.com/community/develop/article/doc/000e88e73a415835ed9b46d7956013
 */
	...
	QRCode.prototype = {
		...
		makeImpl: function(test, maskPattern) {
			...
			if (this.dataCache == null) {
				this.dataCache = QRCode.createData(this.typeNumber, this.errorCorrectLevel, this
					.dataList);
			}
		...
		setupPositionProbePattern: function(row, col) {
			...
			for (var r = -1; r <= 7; r++) {
				...
				for (var c = -1; c <= 7; c++) {
					...
					if (0 <= r && r <= 6 && (c == 0 || c == 6) || 0 <= c && c <= 6 && (r == 0 ||
							r == 6) || 2 <= r && r <= 4 && 2 <= c && c <= 4) {
			...
			for (var row = 0; row < moduleCount; row++) {
				for (var col = 0; col < moduleCount - 6; col++) {
					if (qrCode.isDark(row, col) && !qrCode.isDark(row, col + 1) && qrCode.isDark(
							row, col + 2) && qrCode.isDark(row, col + 3) && qrCode.isDark(row, col +
							4) && !qrCode.isDark(row, col + 5) && qrCode.isDark(row, col + 6)) {
						lostPoint += 40;
					}
				}
			}
			...
			for (var col = 0; col < moduleCount; col++) {
				for (var row = 0; row < moduleCount - 6; row++) {
					if (qrCode.isDark(row, col) && !qrCode.isDark(row + 1, col) && qrCode.isDark(
							row + 2, col) && qrCode.isDark(row + 3, col) && qrCode.isDark(row + 4,
			...
			if (canvas) {
				...
				for (var row = 0; row < qrcode.getModuleCount(); row++) {
					for (var col = 0; col < qrcode.getModuleCount(); col++) {
						...
						ctx.fillRect(Math.round(col * tileW) + options.padding, Math.round(row * tileH) +
							options.padding, w, h);
					}
				}
				...
				ctx = options._this ? uni.createCanvasContext && uni.createCanvasContext(options.canvasId,
					options._this) : uni.createCanvasContext && uni.createCanvasContext(options
					.canvasId);
				width = options.width;
				var tileW = (width - options.padding * 2) / qrcode.getModuleCount();
				var tileH = (width - options.padding * 2) / qrcode.getModuleCount();
				...
				if (options.image.imageResource) {
					ctx.drawImage(options.image.imageResource, options.image.dx, options.image.dy, options
						.image.dWidth, options.image.dHeight);
				}
